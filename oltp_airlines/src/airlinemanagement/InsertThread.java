...
public class InsertThread implements Runnable {
    ...
    InsertThread( Object[] input){
        ...
        stops = Integer.parseInt(input[9].toString());
    ...
    public void run() {
        ...
        if(count==stops+1){
            ...
            if(seat_available){
                ...
                sairportid = Integer.parseInt(temp[0].toString());
                dairportid = Integer.parseInt(temp[1].toString());
                PaymentID = Integer.parseInt(temp[2].toString());
                baseAmt = Double.parseDouble(temp[3].toString());
                taxAmt = Double.parseDouble(temp[4].toString());
                
                if(stops > 0){
                    temp = GetVars(flightName1, basePriceqry1, PaymentMethod, cust_id, miles1);
                    sairportid1 = Integer.parseInt(temp[0].toString());
                    dairportid1 = Integer.parseInt(temp[1].toString());
                    baseAmt1 = Double.parseDouble(temp[3].toString());
                    taxAmt1 = Double.parseDouble(temp[4].toString());
                }
                if(stops > 1){
                    temp = GetVars(flightName2, basePriceqry2, PaymentMethod, cust_id, miles2);
                    sairportid2 = Integer.parseInt(temp[0].toString());
                    dairportid2 = Integer.parseInt(temp[1].toString());
                    baseAmt2 = Double.parseDouble(temp[3].toString());
                    taxAmt2 = Double.parseDouble(temp[4].toString());
                }
                Insert_query();   
            }
        }
...
//                }
//                else{
//                        System.out.println(" 2. Error Calculating the price. Contact Admin.");
//                }
            }
        } catch (Exception e) {
    ...
    private Object[] GetVars(String flightname, String basepriceQry, String paymentmethod, int custid, int mile) {
    ...
    private double[] CalcCost(int dairportid, String basepriceQry, int mile){
        double base =0,tax =0;
    	try {
            int stateID = -1;
            int countryID = -1;
            ...
            if(stateID>0 && countryID==0){
                    ...
                    basetaxQryr = basepriceQry + "  stateid=" + countryID;
            ...
            tax = (double)PGConnection.Run_Query(basetaxQryr);
                ...
                // TODO Auto-generated catch block
                System.out.println("1. Error Calculating the price. Contact Admin.");
        }
